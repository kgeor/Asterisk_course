...
[~]$ ip address
```
Первый сетевой интерфейс в списке с именем `lo`, с адресом 127.0.0.1 (адрес указан после слова inet), также известным как адрес localhost – loopback-адаптер, используемый для обращения ОС к себе самой, трафик этого адаптера никогда не покидает стек ОС и не попадает в сеть, подсеть 127.0.0.0/8 выделена под loopback, и, соответственно, недоступна извне. Используемый для связи с “внешним миром” адрес следует искать в секции, соответствующей интерфейсу с именем, начинающимся на `eth` или `ens` или `enp`. Этот интерфейс соответствует виртуальному сетевому адаптеру, который можно увидеть в конфигурации сети в настройках ВМ, его необходимо перевести в режим "Сетевой мост" или "Bridge" в настройках виртуальной машины.
...
Контекст отображается в следующем синтаксисе: **user:role:type:level**
...
Теперь проверим, что Asterisk готов принимать вызовы, приходящие по сети, для этого нужно убедиться, что прослушивается порт SIP – 5060 UDP. Получить статистику по открытым портам и соединения можно с помощью нескольких команд, одна из них – *ss*:
...
```
**Вопрос для самоконтроля.** Почему некоторые сообщения SIP (в частности INVITE при инициализации сеанса) клиент отправляет по 2 раза? (внимательно посмотрите на структуру сообщений SIP из захваченного трафика, чтобы понять причину)
...
Первым шагом клиент отправляет серверу сообщение REGREQ с информационными элементами ‘username’ (содержит имя пользователя, под которым нужно зарегистрироваться), ‘refresh’ (период обновления регистрации в секундах) и пустым ‘CallToken’;
...
disallow=all
allow=opus,alaw
register => iaxuser2:secret2@<IP_node1>
```
где `<IP_node1>` IP адрес node1
```
На node2 :
[general]
autokill=yes
...
disallow=all
allow=opus,alaw
register => iaxuser1:secret1@<IP_node2>
```
где `<IP_node2>` IP адрес node2

Рассмотрим конфигурацию node1. Она предписывает узлу node1 зарегистрироваться на узле node2 с IP-адресом IP_node2 под пользователем iaxuser2 с паролем secret2. Параметр autokill=yes завершает неудачно установленные соединения по тайм-ауту.

Для принятия изменений перезапустите службу Asterisk:
```
[~]$ sudo systemctl restart asterisk
```
...
В официальном репозитории AppStream доступна версия MariaDB 11.0, чтобы получить новее, необходимо скачать исходный код и собрать пакет самостоятельно, либо добавить репозиторий разработчика в список репозиториев для dnf и установить пакет стандартным способом, что и будет сделано далее.

Для добавления репозитория MariaDB версии 11.2 создайте файл */etc/yum.repos.d/MariaDB.repo* и добавьте в него следующие строки:
...
```
Запуск службы MariaDB (и установка автозапуска на старте):
```
[~]$ sudo systemctl enable --now mariadb
```
После установки рекомендуется осуществить первичную настройку параметров безопасности MariaDB. По умолчанию пароль пользователя СУБД root *не задан* (просто нажать Enter), в следующих шагах, кроме шага, где потребуется задать новый пароль пользователю root (это пользователь СУБД, НЕ системный root), просто прожимвайте Enter, поскольку подходящий ответ выбран по умолчанию:
```
[~]$ sudo mysql_secure_installation || sudo mariadb-secure-installation
```
**Нужно уточнить, как перевести root от сокетной к парольной авторизации и запрет на вход системному руту**
...
Выход в интерпретатор bash производится следующей командой: \q (либо сочетание Ctrl-D)
...
Далее приведены команды для создания БД и ее конфигурации на языке SQL, который традиционно используется для управления реляционными БД. Обратите внимание, запрос написанный нв SQL заканчивается символом «;». Команды SQL выполняются в интерфейсе СУБД (вход `mariadb -u root -p` ).

Создайте БД с именем asteriskdb с символьной кодировкой UTF8mb4 (полная реализация UTF8) по умолчанию:

>CREATE DATABASE asteriskdb DEFAULT CHARACTER SET utf8mb4;
...
Выберите базу данных для работы с ней (по умолчанию после входа в СУБД база не выбрана):

>USE asteriskdb;

Создайте в выбранной базе таблицы, необходимые Asterisk для хранения своих данных с помощью готовых SQL-скриптов.

>source ~/asterisk-20.6.0/contrib/realtime/mysql/mysql_config.sql;

*Если СУБД возвращает ошибку, убедитесь, что путь до файла существует, проверьте, что версия установленного Asterisk (20.6.0 в примере) правильная, замените `~` на `/home/<USERNAME>` где `<USERNAME>` - имя вашего пользователя ОС.*
...
Укажите  при необходимости прокси для клиента системы версионирования Git:
```
[~]$ git config --global http.proxy http://login:pass@proxy:port
```
Склонируйте репозиторий с исходным кодом в текущую директорию и перейдите в него:
```
[~]$ cd ~
[~]$ git clone https://github.com/MariaDB/mariadb-connector-odbc.git
[~]$ cd mariadb-connector-odbc
```
Выберите код версии 3.2.1:
```
[~]$ git checkout 3.2.1
```
...
В конец файла */etc/odbcinst.ini* добавьте конфигурацию драйвера подключения (коннектора) к СУБД (если уже существует запись с именем [MariaDB], приведите к представленному ниже виду):
...
```
***
В файл /etc/odbc.ini добавьте параметры нового ODBC-подключения к MariaDB:
***
```
[Asterisk-odbc]
...
В начало (перед строкой [asterisk]) файла */etc/asterisk/res_odbc.conf* добавьте конфигурацию подключения через ODBC-коннектор:
***
```
[asteriskdb]
enabled => yes
...
[~]$ sudo systemctl restart asterisk
```
**Проверка состояния ODBC подключения Asterisk к БД (Рисунок 3.10):**
```
...
Так должно быть, но не в Вашем случае (0 из 1), не так ли? Если что-то, что вроде бы должно работать - не работает, и при этом включен SELinux - первым делом стоит проверить именно его, не стал ли он причиной возникшей проблемы.

Для работы с логами, правилами и модулями SELinux существует несколько полезных утилит, пришло время познакомиться с некоторыми из них. Первая в списке - `audit2why`, она выведет в читабельном виде все ошибки (неудачные попытки выполнения действий со стороны каких-либо процессов, не имеющих на то разрешения), которые встречаются в логах аудита (по умолчанию с опцией *-a* смотрит в */var/log/audit/audit.log*). 

Для начала попробуйте выполнить команду `audit2why --version` в консоли. Если возвращается ошибка по типу `command not found`, значит в используемой версии дистрибутива утилита не включена в состав установки и ее потребуется доставить. Есть только одна маааленькая проблема: попробовав выполнить команду вроде `sudo dnf install audit2why`, Вы получите с вероятность 99% ошибку, говорящую о том, что пакетный менеджер не нашел пакета с таким именем. Это произошло по той причине, что утилита audit2why входит в состав другого пакета, и для ее получения потребуется установить этот пакет. Дело за малым - узнать имя пакета и установить его.

К счастью, у dnf есть весьма полезная опция, которая позволяет понять, какой же пакет поставляет с собой требуемую утилиту.
```
[~]$ dnf provides audit2why
Last metadata expiration check: 0:03:45 ago on Mon 19 Feb 2024 01:51:46 PM MSK.
policycoreutils-python-utils-3.5-3.el9_3.noarch : SELinux policy core python utilities
Repo        : appstream
Matched from:
Filename    : /usr/bin/audit2why
```
Судя по полученному ответу, установить потребуется пакет с общим названием *policycoreutils-python-utils*. Установите его самостоятельно.

Получив в свои руки инструментарий, можно начинать анализ проблемы с SELinux. Ппоробуем выбрать из лога все ошибки, как-то связанные с MariaDB (мы сейчас не стали делать выборку по ключевому слову asterisk, тк в этом случае мы получим в выводе слишком много лишней информации по ошибкам, которые на работоспособность asterisk-а не влияют). Добавим еще к выводу 5 строк, следующих после найденной строки с совпадением по шаблону.
```
[~]$ sudo audit2why -a | grep -i -A5 mysql
type=AVC msg=audit(1701786104.130:51): avc:  denied  { write } for  pid=1286 comm="asterisk" name="mysql.sock" dev="dm-0" ino=8716934 scontext=system_u:system_r:asterisk_t:s0 tcontext=system_u:object_r:mysqld_db_t:s0 tclass=sock_file permissive=0
        Was caused by:
...
```
Дело все в том, что SELinux не разрешает подключение (в данном случае запись в сокет) процессу с доменом контекста *asterisk_t* к сокету БД, имеющему тип контекста *mysqld_db_t*. Ниже в выводе приводятся пояснения и рекомендации как решить проблему. 

Воспользуемся рекомендацией от audit2why и сгенерируем загружаемый модуль утилитой `audit2allow` для разрешения доступа на запись процессу Asterisk к файлу сокета MariaDB. 
Для этого надо передать запись из лога аудита с описанием ошибки на вход утилите audit2allow, у каждой записи в файле audit.log есть свой уникальный идентификатор, основанный на времени в UNIX-формате (кол-во секунд, прошедших с 1 января 1970), для вышеприведенного примера части вывода audit2why это будет *1701786104.130:51*. При использовании опции *-m* audit2allow генерирует список разрешающих правил Type Enforcement, далее на их основе сгенерируем модуль, скомпилируем и упакуем как пакет загружаемого модуля ядра, и наконец, загрузим в ядро. 
```
[~]$ sudo grep 1701786104.130:51 /var/log/audit/audit.log | audit2allow -m asterisk_db > asterisk_db.te
[~]$ checkmodule -M -m -o asterisk_db.mod asterisk_db.te && semodule_package -o asterisk_db.pp -m asterisk_db.mod
[~]$ sudo semodule -i asterisk_db.pp
```
Перезапустите службу asterisk (`sudo systemctl restart asterisk`) и проверьте ODBC подключение (`sudo asterisk -rx 'odbc show'`). Иии.. снова не работает, надо еще раз проверить лог SELinux
```
sudo audit2why -a | grep mysql
```
Должна появиться еще одна ошибка, связанная с отсутствием разрешающего правила на действие подключение к сокету (*connectto*). Соберем заново модуль с разрешающими правилами, но сделаем это уже попроще. Способ описанный ранее (с применением опции -m `audit2allow -m <имя_модуля>`) хорош, когда необходимо проверить и внести коррективы в правила (файл .te), чтобы потом на его основе "вручную" собрать модуль; в более простом способе c опцией -M (`audit2allow -M <имя_загружаемого_пакета_модуля>`) загружаемый пакет модуля сгенерируется автоматически на основе полученной информации аудита по принципу "как есть".
```
[~]$ sudo grep mysql /var/log/audit/audit.log | audit2allow -M asterisk_db
[~]$ sudo semodule -i asterisk_db.pp
```
Модуль заново будет загружен.Снова перезапустите службу asterisk и проверьте ODBC подключение. Теперь должно отображаться 1 активное подключение.
**Здесь по-хорошему стоит отметить, что порядок производимых действий при решении проблемы нехватки разрешений на подключение Asterisk к СУБД был далеко не оптимален, а выбран был, чтобы продемонстрировать два метода генерации модуля SELinux для разрешения необходимых действий. Гораздо логичнее было бы перевести SELinux временно в Permissive (`sudo setenforce 0`), перезапустить Asterisk, разом соберя в логе все ошибки (а не идти от одной к другой, сейчас их было всего 2, а если, например, будет 10...), сгенерировать модуль, вернуть SELinux в нормальный режим (`sudo setenforce 1`), загрузить модуль, перезапустить Asterisk*

### 3.7. Перенос конфигурации PJSIP в БД.
В начало файла */etc/asterisk/sorcery.conf* вставьте следующие строки для перевода PJSIP в режим Realtime конфигурации:
...
Далее необходимо установить веб-сервер NGINX (пакет *nginx*). Основная конфигурация NGINX содержится в файле /etc/nginx/nginx.conf, его схема (структура) представлена ниже:
***
```
global options
...
Сначала идут глобальные параметры (global options), которые задают основные параметры работы NGINX, например, от какого пользователя будет запущен процесс, а также количество одновременно запущенных процессов (каждый из рабочих процессов - однопоточен). В секции events описывается, как NGINX должен реагировать на события (входящие подключения), ниже расположена секция http, объединяющая все параметры, связанные с работой протокола HTTP. Последняя директива говорит, что дополнительно в конфигурацию секции http включаются все файлы с расширением .conf из директории */etc/nginx/conf.d*, эти файлы могут как содержать дополнительные параметры http, так и секции server c вложенными секциями location. По умолчанию существует файл */etc/nginx/conf.d/default.conf* который ответственен за настройки сайта по умолчанию, отображение проверочной страницы NGINX после установки. Структура конфигурации сайта представлена ниже.
...
Обычно в секции http находится одна или несколько секций server, каждая секция отвечает за отдельный домен (доменное имя сайта), в секции server размещаются секции location, каждая из которых отвечает за обработку запроса с совпадающим URL.
...
worker_processes  auto;
	...
	Этот параметр оптимизируют производительность рабочих процессов NGINX и их количество под число доступных ядер CPU.
...
Параметр  *use epoll* – устанавливает оптимальный эффективный метод обработки соединений для Linux (тк NGINX поставляется не только для Linux, за дефолт принят метод, нормально работающий на всех требуемых ОС). Когда *multi_accept* выключен, рабочий процесс NGINX за один раз будет принимать только одно новое соединение. При включении рабочий процесс за один раз будет принимать сразу все новые соединения.
...
Если на предыдущем шаге проверка конфигурации успешно пройдена, запустите службу NGINX с автозапуском на старте ОС:
```
[~]$ sudo systemctl enable --now nginx
[~]$ systemctl status nginx
```
*если NGINX не стартует и причина - ошибка доступа к /var/run/nginx.pid, помогите ему победить SELinux, выполнив команду `touch /run/nginx.pid && sudo chcon -u system_u -t httpd_var_run_t /var/run/nginx.pid`, перезапустите службу NGINX.*
>
...
Для работы актуальных версий phpMyAdmin требуется веб-сервер с поддержкой PHP 7.4 или новее; пакет PHP-FPM, отвечающий за обработку php-скриптов по запросу от NGINX можно установить из репозитория Remi:

Включение модуля репозитория PHP версии 8.3 и установка PHP-FPM для Nginx и дополнительных пакетов PHP для phpMyAdmin:
...
В файле /etc/php.ini найдите, раскомментируйте и отредактируйте параметр (отключение автоисправления URL):
```
cgi.fix_pathinfo=0
```
...
Проверьте, что она успешно запустилась.
...
>cd ~
>tar -xzf phpMyAdmin-*
>sudo mv ./phpMyAdmin-5.2.1 /usr/share/
> sudo chown -R nginx. /usr/share/phpMyAdmin-5.2.1/
???? >sudo mkdir /var/www/phpmyadmin/tmp
???? >sudo chown -R nginx. /var/www/phpmyadmin/tmp
***

Переименуйте (с помощью команды **mv**) файл */etc/nginx/conf.d/default.conf* в */etc/nginx/conf.d/default.conf.bkp* создайте в той же папке, где находился дефолтный файл конфигурации новый файл с названием *pma.conf* и разместите внутри него конфигурацию следующего вида:
```
server {
    listen       80;
...
```
/ – location т.н. корень сайта, перехватывает все запросы, если не найдено более точное совпадение URL. Порядок и проритеты обработки location это отдельная непростая тема, здесь поднимать ее не будем.

~ \.php$ – location, определяющий порядок обработки всех запросов к php посредством отправки серверу PHP-FPM. То есть если в запросе от клиента указан файл  .php, либо запрос подразумевает выдачу файла index.php, то запрос отправляется на обработку к PHP-FPM.

Теперь нужно разрешить fhp-fpm обращаться к файлам phpMyAdmin, это можно сделать с помощью установки переключателя
```
sudo semanage fcontext -a -u systyem_u -t usr_t '/usr/share/phpMyAdmin-5.2.1(/.*)?'
sudo restorecon -R /usr/share/phpMyAdmin-5.2.1
...
Продемонстрируем способ получения такого доступа, не зная ни имен пользователей, ни паролей. Откройте окно с запущенной ВМ, выполните перезагрузку и как только появится лого гипервизора, нажимайте ESC, пока не отобразится когда появится меню загрузчика нажмите e. Отобразятся параметры загрузки, загрузочное устройство, параметры инициализации ядра и тд. Найдите строку с параметрами загрузки ядра (строка начинается с linux или linuxefi), замените параметр *ro* на *rw* для монтирования файловой системы в режим чтения-записи, добавьте в самый конец параметров init=/bin/sh и нажмите Ctrl+X для загрузки системы с этими параметрами. ОС загрузится в оболочку от имени пользователя root. Теперь Вы можете совершать любые действия в системе, добавить или просмотреть пользователей в системе (cat /etc/passwd), сменить пароль любому пользователю (`passwd <username>`), внести любое изменение в работу ОС. ЕСЛИ в ОС был включен SELinux в активном режиме, выполните после всех произведенных изменений команду `touch /.autorelabel` для создания в корне файловой системы пустого скрытого файла-маркера, который говорит SELinux, что при загрузке необходимо обновить метки всех файлов (а не сверять с значениями до).
