...
Параметры прокси-сервера прописываются в конфигурационном файле dnf /etc/dnf/dnf.conf (для обратноq совместимлсти с yum существует файл /etc/yum.conf с аналогичным содержимым, можно использовать его вместо dnf.conf), его необходимо открыть в редакторе с правами суперпользователя для внесения изменений:
...
Включите репозиторий CRB (ранее назывался PowerTools) (добавлен в CentOS 8 по умолчанию, но выключен) и установите криптографические пакеты и библиотеки для поддержки шифрования передаваемых голосовых данных:
```
...
-	Resource Modules: убрать res_agi, все пункты с res_ari, res_fax, res_geolocation, res_phoneprov, res_smdi, res_adsi (эти модули не нужны в данной установке и вызывают появление ошибок при запуске);
-	Compiler Flags: LOW_MEMORY, G711_NEW_ALGORITHM, G711_REDUCED_BRANCHING;
-	Core Sound Packages: только RU-WAV.
Пометка: Если планируете запускать ВМ с Asterisk на каком-либо другом компьютере (отличном от того, на котором собираете Asterisk), снимите галку BUILD_NATIVE в Compiler Flags
...
sudo make install
```
Создание службы systemd для Asterisk и конфигурационных файлов по умолчанию, файл конфигурации для службы ротации логов logrotate:
...
defaultlanguage = ru
```
Также внесите изменения в /etc/sysconfig/asterisk, раскомментировать строки
```
AST_USER="asterisk"
AST_GROUP="asterisk"
```
...
```
Принудительно заставьте SELinux перечитать маркировки контекста для этих же директорий (при установке был прописан корректный файловый контекст, но проблема в том, что по умолчанию SELinux в момент создания вложенной директории наследует контекст от родительской и автоматически (без перезагрузки) не перечитывает при изменении установленный постоянный контекст (временно сменить можно сразу, без перечитывания командой chcon))
```
...
country=ru
```
Если ранее Asterisk CLI успешно запустился, то можно запускать Asterisk как фоновую службу с автозапуском на старте ОС:
```
...
Установите вспомогательные пакеты и саму СУБД MariaDB :
```
...
```
Проверка возможности локального подключения к СУБД (Пароль вводить от пользователя root СУБД, заданный на предыдущем шаге):mariadb -u root -p (Рисунок 3.7).
...
Далее приведены команды для создания БД и ее конфигурации на языке SQL, который традиционно используется для управления реляционными БД. Обратите внимание, запрос SQL ВСЕГДА заканчивается символом «;». Команды SQL выполняются в интерфейсе СУБД (вход mariadb -u root -p ).
...
>source ~/asterisk-20.5.0/contrib/realtime/mysql/mysql_config.sql;

*Если СУБД возвращает ошибку, убедитесь, что путь до файла существует, проверьте, что версия установленного Asterisk (20.5.0 в примере) правильная, замените ~ на /home/<USERNAME> где <USERNAME> - имя вашего пользователя ОС.*
...
В конец файла /etc/odbcinst.ini добавьте описание нового драйвера подключения к СУБД (если уже существует запись с именем [MariaDB], приведите к представленному ниже виду):
...
Так должно быть, но не в Вашем случае, не так ли? Если у Вас в системе включен SELinux и что-то, что вроде бы должно работать - не работает, первым делом стоит проверить именно его.
Для работы с логами, правилами и модулями SElinux существует несколько утилит, пришло время познакомиться слегка с некоторыми из них. Первая в списке - audit2why, она выведет в читабельном виде все
ошибки (запреты действий), которые встречаются в логах (по умолчанию с опцией -a смотрит в '/var/log/{messages,audit/audit.log}).  
```
[~]$ sudo audit2why -a
...
...
```
Дело все в том, что SELinux не разрешает подключение (в данном случае запись в сокет) процессу с доменом контекста asterisk_t к
сокету БД, имеющему тип  mysqld_db_t.//// Ниже в выводе приводятся пояснения и рекомендации как решить проблему. 

Воспользуемся рекомендацией и сгенерируем загружаемый модуль утилитой audit2allow для разрешения доступа процессу Asterisdk к файлу сокета MariaDB.
Для этого надо передать запись из лога аудита соответствующую проблеме на вход audit2allow, у каждой записи в файле audit.log есть свой уникальный идентификатор, основанный на времени в UNIX-формате (кол-во секунд, прошедших с 1 января 1970), для вышеприведенного примера части вывода audit2why это будет *1701786104.130:51*
```
[ ~]$ sudo grep 1701786104.130:51 /var/log/audit/audit.log | sudo audit2allow -M asterisk_db

!!! Тут вторая ошибка появится еще, сначала надо первую устранить и добавить в te файл правило и требуемые модули 

sudo grep asterisk /var/log/audit/audit.log | audit2allow -m asterisk_db > asterisk_db.te
...
```
Модуль успешно сгенерирован, осталось его активировать (вывод предыдущей команды подсказывает как это сделать, учтите, что требуются root-права).
